...
const guideSections = [
  { title:'快速绑定', items:[
    { label:'1', content:'扫胸牌二维码 → 进入小程序', type:'step' },
    { label:'2', content:'确认员工信息 → 提交', type:'step' },
    { label:'3', content:'非充电状态 → 长按按键＞10 秒', type:'warn' },
    { label:'4', content:'绿灯闪烁 → 屏幕显示姓名 → 绑定成功', type:'done' }
  ]},
  { title:'开始 / 停止录音', items:[
    { label:'●', content:'开启录音：开关上拨（显示绿点）→ 绿灯长亮 2 秒 → 开始录音', type:'on' },
    { label:'●', content:'关闭录音：开关下拨（隐藏绿点）→ 绿灯闪 2 次 → 停止录音', type:'off' },
    { label:'●', content:'隐私保护：仅工作时段可录音，私人时间开关无效', type:'warn' }
  ]},
  { title:'快捷按键', items:[
    { label:'●', content:'录音中（开关 ON）：单击 → POI 重点标记', type:'action' },
    { label:'●', content:'未录音（开关 OFF）：单击 → 手动上传数据', type:'upload' },
    { label:'●', content:'查设备版本信息：开关 OFF + 长按 ＞5 秒 → 显示 SN / 软硬件版本', type:'info' }
  ]},
  { title:'更新 / 解绑员工信息', items:[
    { label:'1', content:'开关拨到OFF（隐藏绿点）', type:'step' },
    { label:'2', content:'非充电状态 → 长按按键 ＞10 秒', type:'step' },
    { label:'3', content:'绿灯闪烁 → 等待平台同步', type:'step' },
    { label:'4', content:'成功：屏幕更新 / 恢复二维码', type:'done' }
  ]},
  { title:'数据上传（3 种方式）', items:[
    { label:'1', content:'平台配置自动周期上传', type:'upload' },
    { label:'2', content:'充电时自动上传', type:'upload' },
    { label:'3', content:'开关 OFF → 单击按键立即上传', type:'upload' }
  ]},
  { title:'指示灯一看就懂', items:[
    { label:'●', content:'绿灯快闪：上传 / 绑定中', type:'led green' },
    { label:'●', content:'绿灯慢闪：录音中', type:'led green' },
    { label:'●', content:'绿灯长亮 2 秒：开始录音', type:'led green' },
    { label:'●', content:'绿灯闪 2 次：停止录音', type:'led green' },
    { label:'●', content:'红灯慢闪：电量低（＜10%）', type:'led red' },
    { label:'●', content:'红灯快闪：温度异常', type:'led red' },
    { label:'●', content:'红灯常亮：充电中', type:'led red' },
    { label:'●', content:'绿灯常亮：充电完成', type:'led green' }
  ]},
  { title:'充电与复位', items:[
    { label:'●', content:'充电：红灯亮 → 充满绿灯亮', type:'charge' },
    { label:'●', content:'复位：连接充电 → 长按 ＞10 秒 （不删录音）', type:'warn' }
  ]},
  { title:'到期提醒', items:[
    { label:'●', content:'胸牌无提示', type:'info' },
    { label:'●', content:'小程序 / SaaS 平台显示到期日', type:'warn' },
    { label:'●', content:'到期后：仅可充电、解绑，无法录音', type:'off' }
  ]}
]
</script>
