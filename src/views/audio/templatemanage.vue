  ...
  <div class="template-manage">
    ...
    <el-dialog v-model="showPreview" :title="'预览效果 - ' + (previewTpl?.name || '')" width="95%" top="2vh" destroy-on-close>
      <div v-if="previewData" class="preview-container">
        ...
        <!-- 销售复盘 → 复盘报告 -->
        <SalesReviewView v-if="isReviewPreview" :data="previewData" />
        <!-- 面包店零售分析 → 面包店专属报告 -->
        <BakeryView v-if="isBakeryPreview" :data="previewData" />
        <!-- 零售客户画像 → 零售分析报告 -->
...
const templateList = ref([
  ...
  { id: 10, name: '教育行业课堂总结', icon: '📚', color: '#f0f9eb', templateType: 'report', hasScore: false, description: '教育行业课堂总结模板，AI自动分析课堂教学内容，生成结构化知识点、举例和练习题', industries: ['教育培训'], scenes: ['课堂授课'], totalDims: 0, totalWeight: 0, scoreRange: '-', enterpriseCount: 1, useCount: 45, avgScore: '-', enabled: true, dimensionConfig: [], sections: [], aiPrompt: '分析课堂教学内容...' },
