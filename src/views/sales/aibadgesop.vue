...
function loadData(){try{const d=JSON.parse(localStorage.getItem(SK));if(d&&d.length)phases.value.forEach((p,pi)=>{if(d[pi]){if(d[pi].name)p.name=d[pi].name;p.tasks.forEach((t,ti)=>{if(d[pi].tasks&&d[pi].tasks[ti]){const dt=d[pi].tasks[ti];if(dt.script)t.script=dt.script;if(dt.psychology)t.psychology=dt.psychology;if(dt.demo)t.demo=dt.demo;if(dt.checkpoint)t.checkpoint=dt.checkpoint;if(dt.duration)t.duration=dt.duration;t._done=dt._done}})}})}}catch(e){}}
